...
### Task 10: Simplify hook configuration

**Files:**
- Modify: `~/.claude/settings.json` (hooks section only)

- [ ] Step 1: Open `~/.claude/settings.json` and replace the entire `hooks` key with:
...
This removes all `PreToolUse` entries. Only the PostToolUse index hook remains.

- [ ] Step 2: Verify no PreToolUse key exists in the hooks section

---

### Task 11: Clean up old enforcement files

- [ ] Step 1: Delete the PreToolUse hook scripts (no longer needed)

```bash
rm -f ~/.claude/hooks/jcodemunch_read_guard.sh
rm -f ~/.claude/hooks/jcodemunch_edit_guard.sh
rm -f ~/.claude/hooks/jcodemunch_read_guard.log
rm -f ~/.claude/hooks/jcodemunch_edit_guard.log
```

- [ ] Step 2: Remove the jCodeMunch policy section from `~/.claude/CLAUDE.md`

Remove the `## Code Exploration Policy` section and everything under it. If that was the only content, delete the file entirely.

- [ ] Step 3: Verify only `jcodemunch_index_hook.sh` remains in `~/.claude/hooks/`

```bash
ls ~/.claude/hooks/jcodemunch_*
```

Expected: only `jcodemunch_index_hook.sh` (and optionally its `.log` file)

---

### Task 12: Copy rewrites and restart

- [ ] Step 1: Copy the 8 rewritten files from `2174/` to wherever tweakcc reads active prompts

The user handles this step manually — the exact copy target depends on tweakcc configuration.

- [ ] Step 2: Restart Claude Code

Close and reopen Claude Code. Hooks and system prompts are loaded at startup.

- [ ] Step 3: Verify prompt integration by asking Claude Code:

> "How do you explore code in this project?"

Expected: response mentions jCodeMunch tools (search_symbols, get_file_outline, get_symbol_source, etc.), NOT "I'll use Grep" or "Let me Read the file."

- [ ] Step 4: Verify non-code routing by asking:

> "Read the README.md"

Expected: uses native Read tool (not jCodeMunch)

- [ ] Step 5: Verify session start by opening a new session in a project directory

Expected: first action is `resolve_repo` with the current directory

- [ ] Step 6: Verify post-edit hook by editing any source code file

Expected: `jcodemunch_index_hook.sh` fires (check log with `JCODEMUNCH_DEBUG=1`)
